...
class PointFusion(nn.Module):
    ...
    def forward(self, x_point, x_image, x_2dbox):
        ...
        lidar_feat = self.feature_transform_forward(g.transpose(2,1))
        ...
        return fused_mu, fused_logvar, boxes, lidar_feat #enc_feat, image_features
...
# Initialize lists to store MAE and R-squared for each epoch
train_mae_scores = []
train_r2_scores = []
valid_mae_scores = []
valid_r2_scores = []
...
for epoch in range(1, epo + 1):
    ...
    # Initialize variables to store epoch-level precision and F1 score for training and validation
    epoch_train_mae = 0.0
    epoch_train_r2 = 0.0
    epoch_valid_mae = 0.0
    epoch_valid_r2 = 0.0
    ...
    for step, (x_points,x_images,y_boxes3d,y_boxes2d) in enumerate(progressbar):
        ...
        mu, logvar, boxes, lidar_feat = model(x_points,x_images,y_boxes2d)
        #print(enc_feat)
        
        
       # Calculate MAE and R-squared for the training set
        y_true_train = y_boxes3d.cpu().numpy()  # Convert to NumPy array
        y_pred_train = boxes.cpu().detach().numpy()  # Convert to NumPy array
        y_true_train_flat = y_true_train.reshape(-1)  # Flatten the array
        y_pred_train_flat = y_pred_train.reshape(-1)  # Flatten the array
        
        train_mae = mean_absolute_error(y_true_train_flat, y_pred_train_flat)
        train_r2 = r2_score(y_true_train_flat, y_pred_train_flat)
        ...
        feature_size = lidar_feat.size(1)
    ...
    if epoch == epo:  # Check if it's the last epoch
        ...
        lidar_feat_list = torch.cat(lidar_feat_list, dim=0)
        ...
        torch.save(lidar_feat_list, 'fuse_feat_'+condition+'.pt')
    model.eval()
    ...
    for step, (x_points,x_images,y_boxes3d,y_boxes2d) in enumerate(progressbar):
        ...
        # Calculate MAE and R-squared for the validation set
        y_true_valid = y_boxes3d.cpu().numpy()  # Convert to NumPy array
        y_pred_valid = boxes.cpu().detach().numpy()  # Convert to NumPy array
        y_true_valid_flat = y_true_valid.reshape(-1)  # Flatten the array
        y_pred_valid_flat = y_pred_valid.reshape(-1)
        
        valid_mae = mean_absolute_error(y_true_valid_flat, y_pred_valid_flat)
        valid_r2 = r2_score(y_true_valid_flat, y_pred_valid_flat)
        
        epoch_valid_mae += valid_mae
        epoch_valid_r2 += valid_r2
    ...
    # Calculate average MAE and R-squared for the epoch
    epoch_train_mae /= len(point_train_batches)
    epoch_train_r2 /= len(point_train_batches)
    epoch_valid_mae /= len(point_val_batches)
    epoch_valid_r2 /= len(point_val_batches)
    
    # Append to lists
    train_mae_scores.append(epoch_train_mae)
    train_r2_scores.append(epoch_train_r2)
    valid_mae_scores.append(epoch_valid_mae)
    valid_r2_scores.append(epoch_valid_r2)
    
    print(f"Training MAE: {epoch_train_mae}, Training R-squared: {epoch_train_r2}")
    print(f"Validation MAE: {epoch_valid_mae}, Validation R-squared: {epoch_valid_r2}")
    ...
    print(textcolors.DEFAULT + "(Average training) Total Loss: {:.3f} | Box MSE,KLD: {}".format(avg_train_loss,train_box))
    ...
    if min_validation_loss > avg_val_loss:
        ...
        print(textcolors.DEFAULT + "New minimum validation loss--model saved!")
...
np.save('train_mae_'+condition+'.npy', np.array(train_mae_scores))
np.save('train_r2_'+condition+'.npy', np.array(train_r2_scores))
np.save('valid_mae_'+condition+'.npy', np.array(valid_mae_scores))
np.save('valid_r2_'+condition+'.npy', np.array(valid_r2_scores))
...
plt.legend()
plt.savefig('loss_lidar_'+condition+'.png',bbox_inches='tight',dpi=300)
